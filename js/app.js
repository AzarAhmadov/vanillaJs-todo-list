const inputsField = document.querySelector(".input-text");
const create = document.querySelector('#create')
const pendingNumber = document.querySelector('.number-create');
const itemCheck = document.querySelector('.show-top');
...
const allTask = () => {
    ...
    pendingNumber.innerHTML = pending.length === 0 ? '0' : pending.length.toString();
}

const complete = () => {
    const completed = document.querySelectorAll('.completed')
    numberCompleted.innerHTML = completed.length === 0 ? '0' : completed.length;
    const pending = document.querySelectorAll('.pending')
    totalTask.innerHTML = pending.length === 0 ? '0' : pending.length.toString();
...
const addTodo = (e) => {
    const inputValue = inputsField.value.trim();
    if ((e.key === "Enter" || e.type === "click") && inputValue.length > 0) {
        const listTag = `
        ...
        <button onCLick='deleteItem(this)' id="delete">
        ...
        `
        todoList.insertAdjacentHTML('beforeend', listTag);
        inputsField.value = "";
...
inputsField.addEventListener('keyup', addTodo);